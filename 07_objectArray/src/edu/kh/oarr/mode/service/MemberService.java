package edu.kh.oarr.mode.service;
...
public class MemberService {
	...
	// memberArr의 비어있는 인덱스 번호를 반환해주는 메서드
		...
		for(int i = 0; i < memberArr.length; i++) {
			...
			if(memberArr[i] != null) { // 회원정보가 있을 경우
				...
				if(memberArr[i].getMemberId().equals(memberId) &&
					...
					break; // 더이상 같은 아이디, 비밀번호가 없기떄문에 효율을 위해 for문 종료
				}
			}
		}
...
//회원 정보 수정 기능
	...
	public void searchRegion() {
		...
		for(int i = 0; i < memberArr.length; i++) {
			...
			// 3) memberArr[i]번째 요소에 저장된 지역(region)이
