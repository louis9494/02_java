...
public class SetService {
	...
	public void method() {
		...
		// HashSet 객체 생성
		//Set<String> set = new HashSet<String>(); // 순서 유지 x
		 Set<String> set = new LinkedHashSet<String>(); // 순서 유지
		// Set<String> set = new TreeSet<String>();
		// 다형성 중 업캐스팅 적용
		
		// 1. boolean (반환형 타입) add(E e)  (메서드명) 추가
		set.add("네이버");
		set.add("카카오");
		set.add("라인");
		set.add("쿠팡");
	...
	public void method2() {
		...
		// 1. Iterateor (반복자)
		// - 컬렉션 객체에 저장된 요소를
		// 	 하나씩 순차 접근하는 객체(인터페이스) 
		
		// Iterateor Set.iterator() : 이터레이터 객체를 반환
		// 반복 접근한 것을 꺼내올 수 있다
		...
		while(it.hasNext()) {
			...
			// E Iterator.next() : 다음 요소를 꺼내와 반환(모든 객체)
			String temp = it.next();
		...
		List<String> list = new ArrayList<String>(set); // 매개변수 set
		...
		for(String snack : set) {// hashCode와 equals를 사용
