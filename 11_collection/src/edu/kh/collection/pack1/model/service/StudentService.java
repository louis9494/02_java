...
public class StudentService {
	...
	public StudentService() {
		...
		studentList.add(new Student("오미나", 27, "충북 청주구", 'F', 90));
	...
	public String addStudent() {
		...
		if( studentList.add(new Student(name, age, region, gender, score))) {
			//(반환형) 					-> 제네릭 때문에 list에 추가할 수 있는 객체 타입이 
			//							student로 제한됨
	...
	public String removeStudent() throws InputMismatchException {
		...
		public void searchName1() {
			System.out.println("===학생 검색(이름이 모두 일치해야 합니다)===");
			...
			String input = sc.next();
			 ...
			 for(Student std : studentList) {
				 ...
				 if( input.equals(std.getName()) ) { // 이름이 일치하는 경우
					 System.out.println( std ); // std.toString(); 하고 같음
		...
		public void searchName2() {
			...
			String input = sc.next();
			 ...
			 // 향상된 for문
			 for(Student std : studentList) {//studentList를 순차접근
				 // 순서 유의!
				 // boolean String.contains(문자열) : String에 문자열이 포함되어 true / 없으면 false
	...
	public void sortByName() {
		...
		// comparing() 는 기본적으로 오름차순 -> .reversed(); 내림차순
		// Comparator 인터페이스의 statuc 메서드인 comparing() 을 사용하여 
		// Comparator 객체를 생성
		// -> comparing() 는 주어진 키를 기반으로 객체를 비교함.
		// Student::getName 	->메서드 레퍼런스(Method Reference)
		// Student 클레스의 getName() 메서드를 가리키는 것.
		// -> 이 메서드를 비교의 키로 사용하여 각Student 객체를 비교하고 정렬함.
	...
	/*public void ex() {
		...
		// List.add(Object e) : 리스트에 객체를 추가할때 쓰는 메서드
