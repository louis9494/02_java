...
public class FileService {
	 ...
	 * - File 클래스 메서드
	 *
	 * boolean  mkdir()          : 디렉토리(폴더) 생성
	 * boolean  mkdirs()         : 경로상의 모든 디렉토리(폴더) 생성
	 ...
	 */
	public void method2() {
		// Java에서 역슬레시 ()는 이스케이프 문자로 사용됨
		// -> \n 줄바꿈, \t 탭과 같이 특정 문자 뒤에 특별한 의미를 부여하는 역할을 함
		// -> 역슬레시 자체를 문자로 사용하고 싶을땐 2개를 사용 \\
		...
		if( !file.exists()) { // 존재하지 않으면
			...
			try {
				...
				// -> IoException은 반드시 예ㅗ이처리를 해야하는 Checked Exception
				// -> 예외처리 필수
	...
	public void method3() {
		...
		//File 객체 생성
		File directory = new File("\\workspace\\02_Javaa\\12_io");
		...
		for( File f : files ) {
			...
			// String SipleDateFormat.format(long) :
			// 매개변수 long 값을 지정된 패턴 형식으로 벼환하여
			// 문자열 형태로 반환
	 ...
	 * 파일 시스템과의 일반적인 상호작용이기 떄문에 스트림 사용할 필요가 없음.
	 * 
	 * 
	 * 
	 * */
