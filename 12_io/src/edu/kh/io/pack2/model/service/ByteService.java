...
public class ByteService {
	 ...
	 * 
	 * - 최상위 클래스 : INputSstream, OutputStream
	 * 
	...
	public void fileByteOutput() {
		...
		// FileOutputStream 참조변수 선언 ( OutputStream 을 상속 받은 자식 클래스 )
		FileOutputStream fos = null; // 기본값으로 null
		...
		try {
			...
			sb.append("!~@#$%^&*()_+?\n");
			sb.append("가나다라마바사아\n");
			sb.append("DFJLSDJKFSFK");
			sb.append("오늘 '하루'만");
			sb.append("화이팅!");
			
			// 출력방법 1: 한 글자(2byte)씩 파일로 출력
			// -> 2byte 범주의 문자(영어, 숫자, 기본 특수문자는 제외)는 바이트 기반 스트림 통과 시
			...
			*/
			...
			// ********수행 시간 확인 ***********
			// 이시점 write() 수행 전 시간
			// Stystem.nanoTime(0 : 1970.01.01 09:00:00 부터
			...
			System.out.println("[수행 시간] : " + (endTime - startTime) + "ns"); // 61000ns
			...
			if(fos != null)
				try {
					...
					// close() 메서드도 IOExcetion 발생하기 때문에
	...
	public void bufferedFileByteOutput() {
		...
		try {
			...
			// 기반 스트림 fos를 이용해 보조 스트림 bos를 생성 << 보조 스트림은 매개변수로 기반 스트림을 만들어야함
	...
	public void fileByteInput() {
		...
		try {
			...
			while(true) {
				...
				if(value == -1)break; // 다 읽으면 반복 멈춤
				
				sb.append((char)value); 
				//append에 추가, char로 강제 형변호나 해서 글자 형태로 sb 추가 
	 ...
	 *
	 * InputStream의 경우 ㅗ이부 파일을 읽어올 때 사용
	 ...
	 * 
	 * 3) 읽어온 내용을 같은 경로 위치에 "파일명_copy" 이름울 출력
	 * 		: FileOutputStream + BufferedOutputStream
	 * 
	 * + Scanner 대신 BufferedReader 이용 -> 문자용 입력용 스트림
	 */
	public void fileCopy() {
		...
		try {
			...
			String target = br.readLine(); // try catch 안에 입력된 한 줄 읽어오기 
			...
			byte[] bytes = bis.readAllBytes(); // 전부 다 받아와서 byte 계열로 반환
			...
			//  없으면 -1 반환
			int insertPoint = target.lastIndexOf("."); // 22 index
			...
			// 출력용 스트림 생성
			fos = new FileOutputStream(copy); // "io_test/20240926/노래가사.txt" -> 기반
			bos = new BufferedOutputStream(fos);
