...
public class ExceptionService {
	...
	// 예외는 두 종류로 구분됨
	// 1) Unchecked Exception : 선택적으로 예외 처리 해야함 (ex. Runtime Exception..)
	// 2) Checked Exception : 필수적으로 예외 처리 (ec. IOException, 나머지..)
	...
	public void ex3() {
		...
		// try - catch - finally
		// finally : try 구문에서 예외가 발생 하든말든 무조건 
		// 마지막에 수행되는 코드를 작성해주면 됨
		try {
			System.out.print("정수 1 입력 :"); // InputMismatchException
			int input1 = sc.nextInt(); // int형이 아닌 다른형 넣었을때
			...
			System.out.println( e.getClass() ); // 어떤 예외 클래스인가를 알수있음
	...
	public void methodC() throws IOException {
						// throws : 호출한 메서드에게 예외를 던질때 쓰는 키워드
