...
import edu.kh.oop.model.vo.Student;
import edu.kh.oop.model.vo.User;

// ClsService와 Student는 패키지가 다름
public class ClsService /*extends Student*/ {
			// 부모로 Student 클래스를 상속받음
			// Student(부모) -> ClisService(자식)
	
	public void ex1() {
		// 클래스 접근제한자 확인하기
		Student std = new Student();
		// import(어디서든 접근가능) public 이기 때문에
		
		//TextVO = new TestVO();
		// (default) class인 TestVO는 import 자체가 불가능(다른패키지라 불가)
		...
		// Student 클래스를 상속 받았으므로 Student의 protexted 변수인
		// v2는 직접 접근 가능
		// System.out.println( v3); // defalt 다른 패키지라 안됨
		// System.out.println( v4); // 해당클래스가 아닌 다른클래스라 안됨
	...
	public void ex2() {
		 ...
		 * 그리고, static 영역에 생성된 변수는 어디서든지 공유가능(공유 메모리 영역이라고도함)
		 * 
		 * 
		 * 
		 * 
	...
	public void ex4 () {
		// 매개변수 생성자 예제
		User u1 = new User(); // 기본생성자 다른 매개변수 생성자 없으면 꼭 만들어야 함
		...
		System.out.printf("u1 : %s / %s / %s / %d / %c\n", 
				...
				u2.getUserAge(), u2.getUserGender() ); // id, pw를 제외한 나머지는 jvm 기본값
